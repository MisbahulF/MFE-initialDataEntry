...
export const InitialDataEntryList: React.FC<InitialDataEntryListProps> = ({ onNavigate }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const fromBackend: ProspectItem[] = apps.map((app: any) => ({
          ...
          statusPrescreening: app.status === 'Menunggu Data Entry' ? 'DIKIRIM KE PROCESSING' : (app.status || 'Draft IDE'),
        ...
        res = storage.getJSON<ProspectItem[]>(STORAGE_KEYS.IDE_PROSPECTS) || [];
      }
    } catch {
      res = storage.getJSON<ProspectItem[]>(STORAGE_KEYS.IDE_PROSPECTS) || [];
    }
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5] font-sans text-gray-800 flex flex-col text-sm">
      ...
      {/* ── Top Nav ── */}
      <header className="bg-white border-b border-gray-200 shadow-sm px-5 py-2.5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="h-7 w-7 rounded bg-[#005E5D] flex items-center justify-center shrink-0">
            <span className="text-white font-black text-[10px] tracking-tight">BNI</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-[#005E5D]">PROMPT eLO</span>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-6 space-y-4">
        ...
        {/* Toast */}
        {toast && (
          <div className="flex items-center gap-2 bg-teal-50 border border-teal-200 text-teal-800 text-xs px-4 py-2.5 rounded-lg">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-[#005E5D] hover:bg-[#004a49] text-white text-xs font-semibold rounded-lg transition-colors"
          >
            <Plus className="h-3.5 w-3.5" /> Input Baru
          </button>
          <button
            onClick={() => onNavigate('form', { isNew: true, isFlpp: true, fasilitas: 'GRIYA FLPP PEMERINTAH' })}
            className="flex items-center gap-1.5 px-4 py-2 border border-[#005E5D] text-[#005E5D] hover:bg-teal-50 text-xs font-semibold rounded-lg transition-colors"
          >
            FLPP
          </button>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-xs overflow-hidden">
          <div className="bg-[#005E5D] px-5 py-2.5">
          ...
          <form onSubmit={handleSearch} className="p-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
              {[
                ...
                <div key={f.label} className="flex items-center gap-3">
                  <label className="w-28 text-xs font-medium text-gray-500 shrink-0 text-right">{f.label}</label>
                  <input
                    type={f.type}
                    value={f.val}
                    onChange={e => f.set(e.target.value)}
                    placeholder={f.ph}
                    className="flex-1 px-3 py-1.5 text-xs border border-gray-300 rounded-lg bg-[#fffef7] focus:outline-none focus:ring-2 focus:ring-[#005E5D]/20 focus:border-[#005E5D] transition"
                  />
                </div>
              ))}

              <div className="flex items-center gap-3">
                <label className="w-28 text-xs font-medium text-gray-500 shrink-0 text-right">Fasilitas</label>
                <select
              ...
              <div className="flex items-center gap-3">
                <label className="w-28 text-xs font-medium text-gray-500 shrink-0 text-right">Sumber Data</label>
                <select
                  value={filterSumber}
            ...
            <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
              ...
              <div className="flex gap-2">
                ...
                <button type="submit" className="flex items-center gap-1.5 px-5 py-1.5 text-xs font-semibold text-white bg-[#005E5D] hover:bg-[#004a49] rounded-lg transition-colors">
        ...
        {/* Results */}
        <div className="bg-white border border-gray-200 rounded-xl shadow-xs overflow-hidden">
          <div className="bg-[#E05A10] px-5 py-2.5">
          ...
          {!hasSearched ? (
            <div className="flex flex-col items-center justify-center py-16 text-gray-400 gap-3">
              ...
              <p className="text-xs text-center">
                Silakan klik tombol <strong className="text-[#005E5D]">"Cari"</strong> di atas untuk menampilkan data prospek.<br />
                Atau klik <button onClick={() => onNavigate('form', { isNew: true })} className="text-[#005E5D] underline font-semibold">Input Baru</button> untuk memulai pencatatan.
              </p>
            </div>
          ) : prospects.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {prospects.map((p, i) => (
                    <tr key={p.noProspek} className={`border-b border-gray-100 hover:bg-teal-50/40 transition-colors ${i % 2 === 1 ? 'bg-gray-50/50' : ''}`}>
                      <td className="px-3 py-2 font-mono text-[#005E5D] font-semibold whitespace-nowrap cursor-pointer hover:underline" onClick={() => onNavigate('form', { ...p, isNew: false })}>{p.noProspek}</td>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="flex items-center gap-1 px-2.5 py-1 text-[10px] font-semibold text-white bg-[#005E5D] hover:bg-[#004a49] rounded-md transition-colors"
                          >
