...
export const SearchZipcodeModal: React.FC<SearchZipcodeModalProps> = ({
  ...
  const handleSearch = async (q: string) => {
    setIsLoading(true);
    try {
      const data = await referenceService.searchZipcode(q);
      setResults(data);
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Ketik nama Kelurahan, Kecamatan, Kota, atau 5 digit Kodepos</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 pb-2">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Cari kelurahan / kecamatan / kodepos (contoh: Serang, Cipare, 42111)..."
        ...
        <div className="p-6 pt-3 max-h-80 overflow-y-auto">
          {isLoading ? (
            <div className="text-center py-8 text-sm text-slate-500">Mencari data wilayah...</div>
          ) : results.length === 0 ? (
            <div className="text-center py-8 text-sm text-slate-400">Data wilayah tidak ditemukan.</div>
          ) : (
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-semibold uppercase">
                <tr>
                  <th className="p-2.5 rounded-l-md">Kodepos</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-200">
                {results.map((r, idx) => (
                  <tr key={idx} className="hover:bg-orange-50/50 dark:hover:bg-slate-800/60 transition">
                    <td className="p-2.5 font-mono font-bold text-[#f15a24]">{r.zipcode}</td>
                    <td className="p-2.5 font-medium">{r.kelurahan}</td>
                    <td className="p-2.5">{r.kecamatan}</td>
                    <td className="p-2.5">{r.kota}</td>
